...
export class CompanyaddeditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.countryService.getCountries().subscribe(
      (data: Country[]) => {
        this.countries = data; // Assuming data.content is the array of countries
      },
      (error) => {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (this.companyId) {
        this.loadCompanyDetails(Number(this.companyId)); // Convert companyId to number
      }
    });
  }
  ...
  loadCompanyDetails(id: number): void {
    this.companyService.getCompanyById(id).subscribe(
      (company) => {
        this.companyForm.patchValue(company);  // Load company details into the form
      },
      (error) => {
  ...
  saveCompany(): void {
    ...
    const companyToSave: Company = { ...this.companyForm.value, id: this.companyId ? Number(this.companyId) : null };
